...
async def get_kundli(
    ...
    try:
        ...
        # Include D10 chart if it exists (generated by jyotishyamitra)
        if 'd10_chart' in kundli_file_data:
            response_data['d10_chart'] = kundli_file_data.get('d10_chart', {})
            print(f"[GET_KUNDLI] D10 chart included in response")
        
        # Include D10 raw data if it exists
        if 'd10_raw' in kundli_file_data:
            response_data['d10_raw'] = kundli_file_data.get('d10_raw', {})
...
class KundliDataExtractor:
    ...
    def extract_comprehensive_kundli_data(kundli_data: dict) -> dict:
        ...
        # 1. Top-level in kundli_data (most common in current structure)
        divisional_charts = kundli_data.get('divisionalCharts', {})
        
        # 2. Inside jyotishganit_json
        if not divisional_charts:
            divisional_charts = kundli_data.get('jyotishganit_json', {}).get('divisionalCharts', {})
        
        # 3. Inside comprehensive_kundli (old structure)
        if not divisional_charts:
            comprehensive_kundli = kundli_data.get('comprehensive_kundli', {})
            divisional_charts = comprehensive_kundli.get('divisionalCharts', {})
        
        # 4. Inside comprehensive_kundli.jyotishganit_json
        if not divisional_charts:
            comprehensive_kundli = kundli_data.get('comprehensive_kundli', {})
        ...
        # Extract D1 chart data if available (main birth chart)
        # D1 is the foundational Rashi chart - check multiple locations
        d1_chart = kundli_data.get('d1Chart', {})
        
        # If not found at top level, check in jyotishganit_json
        if not d1_chart:
            d1_chart = kundli_data.get('jyotishganit_json', {}).get('d1Chart', {})
        
        # If still not found, check in comprehensive_kundli
        if not d1_chart:
            comprehensive_kundli = kundli_data.get('comprehensive_kundli', {})
            d1_chart = comprehensive_kundli.get('d1Chart', {})
        
        # If still not found, check in comprehensive_kundli.jyotishganit_json
        if not d1_chart:
            comprehensive_kundli = kundli_data.get('comprehensive_kundli', {})
            d1_chart = comprehensive_kundli.get('jyotishganit_json', {}).get('d1Chart', {})
        
        if d1_chart:
            extracted_data["divisional_charts"]["d1"] = d1_chart
            if "D1" not in extracted_data["divisional_charts"]["available_charts"]:
                extracted_data["divisional_charts"]["available_charts"].insert(0, "D1")
                extracted_data["divisional_charts"]["total_charts"] += 1
            print(f"[KUNDLI_EXTRACTOR] ✓ D1 (Rashi/Birth Chart) found and added")
        else:
            print(f"[KUNDLI_EXTRACTOR] ℹ D1 chart not found in separate location (D1 data is in horoscope_analysis as foundational chart)")
        
        # Extract D10 chart data if available
        # Check multiple locations for D10 chart
        d10_chart = kundli_data.get('d10_chart', {})
        
        # If not found at top level, check in jyotishganit_json
        if not d10_chart:
            d10_chart = kundli_data.get('jyotishganit_json', {}).get('d10_chart', {})
        
        # If still not found, check in divisionalCharts (D10 might be stored there)
        if not d10_chart:
            d10_chart = kundli_data.get('divisionalCharts', {}).get('d10', {})
        
        # If still not found, check in comprehensive_kundli
        if not d10_chart:
            comprehensive_kundli = kundli_data.get('comprehensive_kundli', {})
            d10_chart = comprehensive_kundli.get('d10_chart', {})
        
        # If still not found, check in comprehensive_kundli.jyotishganit_json
        if not d10_chart:
            comprehensive_kundli = kundli_data.get('comprehensive_kundli', {})
            d10_chart = comprehensive_kundli.get('jyotishganit_json', {}).get('d10_chart', {})
        
        if d10_chart:
            extracted_data["divisional_charts"]["d10"] = d10_chart
            if "D10" not in extracted_data["divisional_charts"]["available_charts"]:
                extracted_data["divisional_charts"]["available_charts"].append("D10")
                extracted_data["divisional_charts"]["total_charts"] += 1
            print(f"[KUNDLI_EXTRACTOR] ✓ D10 (Dasamsa/Career Chart) found and added (source: jyotishyamitra)")
        else:
            print(f"[KUNDLI_EXTRACTOR] ℹ D10 chart not found (checked: d10_chart, jyotishganit_json.d10_chart, divisionalCharts.d10, comprehensive_kundli.d10_chart)")
        
        # Print final summary
        print(f"[KUNDLI_EXTRACTOR] Total charts available: {extracted_data['divisional_charts'].get('total_charts', 0)}")
        print(f"[KUNDLI_EXTRACTOR] All available charts: {extracted_data['divisional_charts'].get('available_charts', [])}")
...
Please provide a thorough, detailed response covering all aspects of the matching analysis."""
            ...
            # Build comprehensive divisional chart instructions
            divisional_charts_info = comprehensive_data.get('divisional_charts', {})
            available_charts = divisional_charts_info.get('available_charts', [])
            
            divisional_chart_instructions = ""
            if available_charts:
                divisional_chart_instructions = f"""

IMPORTANT DIVISIONAL CHARTS INFORMATION:
The comprehensive kundli data includes the following divisional charts: {', '.join(available_charts)}

These charts are available in the 'divisional_charts' object in the data. Each chart (D2, D3, D4, D5, D6, D7, D8, D9, D10, D12, D16, D20, D24, D27, D30, D40, D45, D60) contains:
- ascendant: The ascendant sign and D1 house placement
- houses: Array of houses with sign, lord, and occupants (planets in that house)
- Each planet shows: celestialBody name, sign, and d1HousePlacement

When the user asks about ANY divisional chart (D2, D3, D4, D5, D6, D7, D8, D9, D10, D12, D16, D20, D24, D27, D30, D40, D45, D60):
1. ALWAYS check the divisional_charts object in the provided data
2. Extract the relevant chart data (e.g., 'd2', 'd3', 'd9', 'd10', etc.)
3. Analyze the planets, houses, and their placements in that chart
4. Provide detailed insights based on the actual chart data

DO NOT say that a divisional chart is not available if it exists in the divisional_charts object.
DO NOT make up chart data - only use the data provided in the divisional_charts object.

Available charts to analyze: {', '.join(available_charts)}"""
            
            # Add D10 specific instructions if available
            d10_instructions = ""
            if 'd10' in [c.lower() for c in available_charts]:
                d10_instructions = """

SPECIFIC D10 (DASAMSA) CHART INSTRUCTIONS:
D10 is crucial for career, profession, and public image analysis. When asked about career, profession, or D10:
- Analyze the D10 ascendant and its strength
- Check planets in the 10th house of D10 (career house)
- Analyze the 10th lord in D10
- Consider planetary placements and their dignity in D10
- Use this data to provide career insights"""
...
{json.dumps(comprehensive_data, indent=2, default=str)}{divisional_chart_instructions}{d10_instructions}
