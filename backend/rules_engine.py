...
class RulesEngine:
    ...
    def get_planet_name(self, planet: Dict) -> str:
        ...
        Uses normalized planet names for consistency and case-insensitivity.
        
        Args:
            planet: Planet data dictionary
            
        Returns:
            Planet name (e.g., "sun", "jupiter", "rahu") - normalized lowercase
        """
        # Try different field names
        name = planet.get("name", "") or planet.get("celestialBody", "") or planet.get("planet", "")
        # Normalize the planet name (handles case-insensitivity, aliases, symbol removal)
        return normalize_planet(name)
    ...
    def detect_all_doshas(self, d1_chart: Dict, birth_data: Dict, kundli_data: Dict = None) -> List[Dosha]:
        """
        ...
        Args:
            ...
            kundli_data: Full kundli data (optional, for access to panchanga and horoscope_info)
        ...
        doshas.append(self.detect_gandmool_dosha(d1_chart, kundli_data))
    ...
    def detect_mangal_dosha(self, d1_chart: Dict, birth_data: Dict) -> Dosha:
        ...
        try:
            ...
            for planet in planets:
                ...
                if match_planet(planet_name, ["mars"]) or planet.get("id") == 2:
                    mars_house = extract_house_number(planet.get("house"))
    ...
    def detect_kaal_sarp_dosha(self, d1_chart: Dict) -> Dosha:
        ...
        try:
            ...
            for planet in planets:
                ...
                house = extract_house_number(planet.get("house"))
                
                # Find Rahu and Ketu
                if match_planet(planet_name, ["rahu"]) or planet_id == 7:
                    rahu_house = house
                elif match_planet(planet_name, ["ketu"]) or planet_id == 8:
                    ketu_house = house
    ...
    def detect_pitra_dosha(self, d1_chart: Dict) -> Dosha:
        ...
        try:
            ...
            for planet in planets:
                ...
                if planet_id == 0 or match_planet(planet_name, ["sun"]):
                    sun_house = house
                elif planet_id == 6 or match_planet(planet_name, ["saturn"]):
                    saturn_house = house
                elif planet_id == 7 or match_planet(planet_name, ["rahu"]):
                    rahu_house = house
    ...
    def detect_guru_chandal_dosha(self, d1_chart: Dict) -> Dosha:
        ...
        try:
            ...
            for planet in planets:
                ...
                if planet_id == 4 or match_planet(planet_name, ["jupiter"]):
                    jupiter_house = house
    ...
    def detect_kemadruma_dosha(self, d1_chart: Dict) -> Dosha:
        ...
        try:
            ...
            for planet in planets:
                ...
                if planet_id == 1 or match_planet(planet_name, ["moon"]):
                    moon_house = house
    ...
    def detect_grahan_dosha(self, d1_chart: Dict) -> Dosha:
        ...
        try:
            ...
            for planet in planets:
                ...
                if planet_id == 0 or match_planet(planet_name, ["sun"]):
                    sun_house = house
                elif planet_id == 1 or match_planet(planet_name, ["moon"]):
                    moon_house = house
                elif planet_id == 7 or match_planet(planet_name, ["rahu"]):
                    rahu_house = house
                elif planet_id == 8 or match_planet(planet_name, ["ketu"]):
                    ketu_house = house
    ...
    def detect_gandmool_dosha(self, d1_chart: Dict, kundli_data: Dict = None) -> Dosha:
        """
        ...
        Extracts nakshatra from multiple possible locations in the chart data.
        """
        is_present = False
        is_cancelled = False
        cancellation_reasons = []
        ...
        try:
            ...
            # Try multiple locations for nakshatra data
            # Priority 1: jyotishganit_json.panchanga.nakshatra (most common in comprehensive kundli)
            if kundli_data:
                jyotishganit = kundli_data.get("jyotishganit_json", {})
                if "panchanga" in jyotishganit:
                    panchanga = jyotishganit.get("panchanga", {})
                    raw_nakshatra = panchanga.get("nakshatra")
            
            # Priority 2: d1_chart.panchanga.nakshatra (alternative location)
            if not raw_nakshatra and "panchanga" in d1_chart:
                panchanga = d1_chart.get("panchanga", {})
                raw_nakshatra = panchanga.get("nakshatra")
            
            # Priority 3: horoscope_info.nakshatra (alternative location)
            if not raw_nakshatra and kundli_data:
                horoscope_info = kundli_data.get("horoscope_info", {})
                raw_nakshatra = horoscope_info.get("nakshatra")
            
            # Priority 4: moon_nakshatra from horoscope_info
            if not raw_nakshatra and kundli_data:
                horoscope_info = kundli_data.get("horoscope_info", {})
                raw_nakshatra = horoscope_info.get("moon_nakshatra")
            
            # If still no nakshatra found, return not present
            if not raw_nakshatra:
                return Dosha(
                    name="Gandmool Dosha",
                    is_present=False,
                    is_cancelled=False,
                    severity="mild",
                    description="Nakshatra data not available in chart.",
                    cancellation_reasons=[],
                    remedies=[]
                )
            
            # Safely extract nakshatra with defensive handling
            if isinstance(raw_nakshatra, dict):
                raw_nakshatra = raw_nakshatra.get("name", "")
            
            # Normalize to lowercase and strip whitespace
            nakshatra_str = extract_and_clean_string(raw_nakshatra)
            
            # Check if this is a Gandmool nakshatra using robust matching
            if is_gandmool_nakshatra(nakshatra_str):
                is_present = True
                severity = "moderate"
                # Get the canonical nakshatra name for display
                canonical_name = normalize_nakshatra(nakshatra_str)
                description = f"Gandmool Dosha is present. The Moon is in {canonical_name.title()} nakshatra at birth. This requires specific remedial measures during early childhood. Note: Gandmool Dosha cannot be cancelled out and must be addressed through proper remedial rituals."
        ...
        try:
            ...
            if "planets" in d1_chart:
                for planet in d1_chart["planets"]:
                    planet_name = normalize_planet(planet.get("name", ""))
                    planet_id = planet.get("id")
                    house_name = normalize_rasi(planet.get("house_name", ""))
                    longitude = extract_degree(planet.get("longitude", 0))
                ...
                for planet_name, data in planets_data.items():
                    ...
                    for planet, debil_sign in self.debilitation_signs.items():
                        if match_planet(planet_name, [planet]) and match_rasi(house_name, [debil_sign]):
                ...
                if sun_longitude is not None:
                    for planet_name, data in planets_data.items():
                        if not match_planet(planet_name, ["sun"]):
                            longitude_diff = abs(data["longitude"] - sun_longitude) if data["longitude"] is not None else 0
                ...
                for planet in d1_chart.get("planets", []):
                    ...
                    if is_malefic_planet(planet_name):
        ...
        try:
            ...
            if "planets" in d1_chart:
                for planet in d1_chart["planets"]:
                    ...
                    # Skip Ascendant
                    if match_planet(planet_name, ["ascendant"]):
                        continue
            ...
            for house, planets in dusthana_houses.items():
                if planets:
                    ...
                    malefics_present = any(is_malefic_planet(planet) for planet in planets)
    ...
    def get_planet_sign(self, planet: Dict) -> Optional[str]:
        """
        Extract sign from planet object with normalization.
        
        Uses normalized Rasi names for consistency and case-insensitivity.
        
        Args:
            planet: Planet data dictionary
            
        Returns:
            Sign name (e.g., "aries", "taurus") - normalized lowercase, or None
        """
        sign = planet.get("sign", "")
        if sign:
            return normalize_rasi(sign)
        # Try to extract from house_name if available
        house_name = planet.get("house_name", "")
        if house_name:
            return normalize_rasi(house_name)
        return None
    ...
    def is_benefic_planet(self, planet_name: str) -> bool:
        ...
        """
        return is_benefic_planet(planet_name)
    ...
    def get_kendra_planets(self, d1_chart: Dict, from_house: int, exclude_planets: List[str] = None) -> List[Dict]:
        ...
        try:
            ...
            for planet in planets:
                ...
                # Skip excluded planets using normalized matching
                if any(match_planet(planet_name, [excluded]) for excluded in exclude_planets):
                    continue
    ...
    def check_mangal_dosha_bhanga(self, d1_chart: Dict, mars_house: int, mars_sign: str) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if mars_sign and match_rasi(mars_sign, ["aries", "scorpio"]):
                is_cancelled = True
                reasons.append(f"Mars is in its own sign of {mars_sign}")
            
            # Condition 2: Mars in exaltation (Capricorn)
            if mars_sign and match_rasi(mars_sign, ["capricorn"]):
                is_cancelled = True
            ...
            for planet in planets:
                ...
                if match_planet(planet_name, ["jupiter"]):
                ...
                if match_planet(planet_name, ["moon"]):
                    if self.check_aspect_between_planets(planet, mars_house, d1_chart):
                        is_cancelled = True
    ...
    def check_kaal_sarp_dosha_bhanga(self, d1_chart: Dict, rahu_house: int, ketu_house: int) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            for planet in planets:
                planet_name = self.get_planet_name(planet)
                if match_planet(planet_name, ["ascendant"]) or planet.get("id") == 9:
                    ascendant_house = extract_house_number(planet.get("house"))
                    break
            ...
            for planet in planets:
                planet_name = self.get_planet_name(planet)
                planet_house = extract_house_number(planet.get("house"))
                
                if match_planet(planet_name, ["jupiter", "venus", "mercury"]):
                    rahu_pos = rahu_house if rahu_house else 0
                    ketu_pos = ketu_house if ketu_house else 0
    ...
    def check_conjunction_dosha_bhanga(self, d1_chart: Dict, planet1_house: int, planet1_name: str, planet2_name: str) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            for planet in planets:
                planet_name = self.get_planet_name(planet)
                planet_house = extract_house_number(planet.get("house"))
                if match_planet(planet_name, [planet1_name]) and planet_house == planet1_house:
            ...
            if conjunction_sign:
                ...
                if ruler and match_planet(planet1_name, [ruler]):
                    is_cancelled = True
                    reasons.append(f"Conjunction is in {planet1_name}'s own sign of {conjunction_sign}")
                
                # Check if conjunction is in exaltation sign
                exalt_normalized = normalize_rasi(self.exaltation_signs.get(planet1_name, ""))
                if match_rasi(conjunction_sign, [exalt_normalized]):
                    is_cancelled = True
                ...
                if planet1_name in self.own_signs:
                    ...
                    if match_rasi(conjunction_sign, own_signs_list) and not match_rasi(conjunction_sign, [exalt_normalized]):
                        is_cancelled = True
                        reasons.append(f"Conjunction is in {planet1_name}'s own sign of {conjunction_sign}")
    ...
    def check_pitra_dosha_bhanga(self, d1_chart: Dict) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            for planet in planets:
                ...
                if planet_id == 0 or match_planet(planet_name, ["sun"]):
                    sun_house = house
                elif planet_id == 4 or match_planet(planet_name, ["jupiter"]):
                    jupiter_house = house
