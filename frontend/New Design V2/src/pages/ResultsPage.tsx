...
import { Calendar, Clock, MapPin, Sparkles, Star, Sun, Download, MessageCircle, ChevronDown, ChevronUp, Zap, Globe, ArrowRight } from "lucide-react"
import { Button } from "@/components/ui/button"

const kundliData = {
  name: "Neha Verma", birthDate: "2000-01-01", birthTime: "12:00", latitude: 28.6139, longitude: 77.2090, place: "New Delhi, India",
  panchanga: { tithi: "Krishna Ekadashi", nakshatra: "Swati", yoga: "Sukarma", karana: "Bava", vaara: "Saturday" },
  ayanamsa: { name: "True Chitra Paksha", value: 23.8399943472118 },
  planets: [
  ...
  additionalData: { house1Sign: "Pisces", house1Lord: "Jupiter", house2Sign: "Aries", house2Lord: "Mars", jupiterSign: "Aries" }
}
  ...
  return (
    <div className="group cosmic-card rounded-xl p-4 hover:bg-purple-500/10 transition-all duration-300">
      <div className="flex items-start gap-4">
        <div className="relative"><div className={`w-12 h-12 rounded-full ${planet.color} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform`}><span className="text-xs font-bold text-white">{planet.name.slice(0, 2)}</span></div><div className={`absolute inset-0 rounded-full ${planet.color} opacity-0 group-hover:opacity-50 blur-xl transition-opacity`} /></div>
        ...
        <div className="px-2 py-1 rounded-lg bg-purple-500/20 border border-purple-500/30 text-xs font-mono text-purple-300">{planet.degree.toFixed(1)}°</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="relative space-y-8 min-h-screen pb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div><h1 className="text-4xl font-bold gradient-text-purple text-glow-purple">Kundli Results</h1><p className="text-muted-foreground">Birth Chart Analysis for <span className="text-cyan-400">{kundliData.name}</span></p></div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Button className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 gap-2 glow-cyan"><Download className="w-4 h-4" />Download PDF</Button>
            <Link to="/chat"><Button variant="outline" className="border-purple-500/30 bg-purple-500/10 hover:bg-purple-500/20 gap-2"><MessageCircle className="w-4 h-4" />Chat with AI</Button></Link>
          </div>
        </div>
      </div>

      <div className="relative cosmic-card rounded-2xl overflow-hidden"><div className="absolute inset-0 bg-gradient-to-b from-purple-900/20 to-transparent" /><OrbitingPlanets /><div className="absolute bottom-4 left-4 right-4 text-center"><p className="text-sm text-muted-foreground">Interactive Planetary Visualization</p></div></div>
      ...
      <SectionCard title="Panchanga (Astrological Calendar)" icon={Calendar} gradient="from-purple-500/50 to-pink-500/30">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">{Object.entries(kundliData.panchanga).map(([key, value]) => (<div key={key} className="cosmic-card rounded-xl p-4 text-center hover:bg-purple-500/10 transition-colors"><p className="text-xs text-purple-400 uppercase tracking-wider mb-1">{key}</p><p className="font-semibold text-foreground">{value}</p></div>))}</div>
      </SectionCard>

      <SectionCard title="Ayanamsa (Precession Correction)" icon={Globe} gradient="from-pink-500/50 to-purple-500/30">
        <div className="grid md:grid-cols-2 gap-6"><DataItem label="Name" value={kundliData.ayanamsa.name} /><DataItem label="Value" value={kundliData.ayanamsa.value.toString()} accent /></div>
      </SectionCard>

      <SectionCard title="Houses and Planetary Positions" icon={Sun} gradient="from-yellow-500/50 to-orange-500/30" collapsible>
        <div className="grid md:grid-cols-2 gap-4">{kundliData.planets.map((p) => (<PlanetCard key={p.name} planet={p} />))}</div>
      </SectionCard>

      <SectionCard title="Additional Astrological Data" icon={Sparkles} gradient="from-cyan-500/50 to-blue-500/30" collapsible>
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">{Object.entries(kundliData.additionalData).map(([key, value]) => (<div key={key} className="cosmic-card rounded-xl p-4 hover:bg-purple-500/10 transition-colors"><DataItem label={key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())} value={value} /></div>))}</div>
      </SectionCard>
        ...
        <div className="p-6 bg-gradient-to-r from-purple-900/40 to-cyan-900/20">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <Button className="bg-gradient-to-r from-purple-600 to-cyan-600 hover:from-purple-500 hover:to-cyan-500 gap-2 glow-purple text-lg px-6 py-3 h-auto"><Zap className="w-5 h-5" />Generate AI Analysis<ArrowRight className="w-4 h-4 ml-2" /></Button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="cosmic-card rounded-2xl p-6">
          ...
          <Button className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 gap-2"><Download className="w-4 h-4" />Download PDF Report</Button>
        </div>
        <div className="cosmic-card rounded-2xl p-6">
          ...
          <Link to="/chat"><Button className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 gap-2"><MessageCircle className="w-4 h-4" />Start Chat Session</Button></Link>
        </div>
      </div>
    </div>
  )
