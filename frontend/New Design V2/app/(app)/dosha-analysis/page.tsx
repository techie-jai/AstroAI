...
import { Calendar, AlertTriangle, Info, ChevronDown, ChevronUp, Sparkles, Shield, Activity, Star } from "lucide-react"
      ...
      {[...Array(8)].map((_, i) => (
        <div
          ...
          style={{
            ...
            width: `${60 + i * 10}px`,
            height: `${60 + i * 10}px`,
            background: `radial-gradient(circle, ${i % 2 === 0 ? 'rgba(139, 92, 246, 0.3)' : 'rgba(6, 182, 212, 0.2)'}, transparent)`,
    ...
    <div className="relative space-y-8 min-h-screen">
      ...
      <div className="relative space-y-2">
        ...
        <Select value={selectedKundli} onValueChange={setSelectedKundli}>
          <SelectTrigger className="w-full max-w-md bg-secondary/50 border-purple-500/30 focus:border-purple-500/50">
            <SelectValue />
          </SelectTrigger>
      ...
      <div className="relative grid grid-cols-2 md:grid-cols-4 gap-4">
        {doshaStats.map((stat, index) => (
          ...
          >
            ...
            <p className="text-4xl font-bold text-foreground group-hover:gradient-text-purple transition-all">
              {stat.value}
            </p>
              ...
              <div key={dasha.name} className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${dasha.color} flex items-center justify-center shadow-lg`}>
                      <span className="text-xs font-bold text-white">{dasha.planet.slice(0, 2)}</span>
                    </div>
                    <div>
                      <h4 className="font-semibold text-purple-300 text-glow-purple">{dasha.name}</h4>
                      <p className="text-sm text-muted-foreground">{dasha.period}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-muted-foreground">Ends in</p>
                    <p className="text-sm font-medium text-cyan-400">{dasha.endsIn}</p>
                  </div>
                </div>
                <div className="relative">
                  <Progress value={dasha.progress} className="h-3 bg-secondary/50" />
                  <div 
                    className="absolute top-0 h-3 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 transition-all"
      ...
      <div className="cosmic-card rounded-2xl overflow-hidden">
        ...
        <div className="p-6">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-500/30 to-orange-500/20 border border-amber-500/30 flex items-center justify-center animate-pulse-glow">
          ...
          <div className="space-y-4">
            <div className="p-5 rounded-xl bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/30 neon-border-gold">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="font-semibold text-amber-400 text-glow-gold">
            ...
            <div className="p-5 rounded-xl cosmic-card">
              <div className="flex items-start gap-4">
          ...
          <div className="px-6 pb-6 space-y-4">
            {doshas.map((dosha, index) => (
              <div
                key={dosha.name}
                className={cn(
                  "p-5 rounded-xl border transition-all duration-300 hover:scale-[1.01] group",
                  dosha.present
                    ? "bg-gradient-to-r from-red-500/10 to-orange-500/10 border-red-500/30 neon-border-gold"
                    : "cosmic-card hover:bg-purple-500/5"
                )}
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <div className="flex items-center justify-between">
