...
# Add parent directory and PyJHora to path - MUST be done before importing jhora
sys.path.insert(0, os.path.join(os.path.dirname(__file__)))
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'PyJHora'))

# Set ephemeris path BEFORE importing any jhora modules - CRITICAL
...
# Now import jhora modules - only import what's needed
from jhora import const
from jhora.horoscope import main
from jhora.horoscope.chart import charts

# Try to import utils and drik, but continue if not available
try:
    from jhora import utils
    utils.set_language('en')
except ImportError:
    print("⚠️ utils module not available, continuing without language setting")
except AttributeError:
    print("⚠️ utils.set_language not available, continuing without language setting")

try:
    from jhora.panchanga import drik
except ImportError:
    print("⚠️ drik module not available, continuing without panchanga")

# Patch missing resource_strings if needed
try:
    if not hasattr(utils, 'resource_strings'):
        print("⚠️ Patching missing utils.resource_strings attribute")
        utils.resource_strings = {}
except:
    pass
