...
class JyotishganitChartAPI:
        ...
        for i, planet in enumerate(self.current_chart.d1_chart.planets):
            if i < len(planet_names):
                ...
                planet_info = {
                    ...
                    'nakshatra_lord': self._get_nakshatra_lord(planet.nakshatra),
    ...
    def _generate_divisional_chart(self, chart_type: str, chart_name: str, signification: str) -> Dict:
        ...
        for i, planet in enumerate(divisional_chart.planets):
            if i < len(planet_names):
                planet_name = planet_names[i]
    ...
    def get_kundli(self) -> Dict:
        ...
        try:
            ...
            # Add our processed charts (simplified version to avoid serialization issues)
            charts = {}
            for chart_type in ['D1', 'D9', 'D10']:  # Only essential charts
                try:
                    chart_data = self.get_chart(chart_type)
                    if 'error' not in chart_data:
                        # Simplify chart data for JSON serialization
                        simplified_chart = {
                        ...
                        charts[chart_type] = simplified_chart
                except Exception as e:
    ...
    def _extract_dashas(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'current': getattr(dashas, 'current', None)
        }
    ...
    def format_chart_text_old(self, chart_type: str) -> str:
        """Format chart data as readable text - old method for compatibility"""
        chart_data = self.get_chart(chart_type)
        return self.format_chart_text(chart_data)
